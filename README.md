...
- angle labels contain `data-angle-label-render-profile="mathjax-3.2.2-chtml-tex-scale1-css-px-v1"`, use CommonHTML rather than nested SVG output, and have a computed font size of exactly `18px`
- side labels also have a computed font size of exactly `18px` and remain clearly separated from their triangle sides at the configured `22px` center-line offset
