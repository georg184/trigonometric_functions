...
Current vendored helper version: `0.4.24`.
...
The app independently pins helper `0.4.24`, calibration `angle-label-tuning-v35`, and data cloud `angle-label-data-cloud-v24` in `EXPECTED_ANGLE_LAYOUT_CONTRACT`. `assertAngleLabelCalibrationContract()` runs before quiz generation, so a stale vendored helper fails explicitly rather than silently using a different model.

GitHub Pages runs `node scripts/verify-angle-layout-consumer.js` before packaging the site. This CI check compares the app expectation with the vendored helper, validates the render profile and every static asset cache token, and rejects deployment on drift.
